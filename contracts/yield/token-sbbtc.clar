...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? sbBTC amount recipient)
  )
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (try! (ft-transfer? sbBTC amount sender recipient))
    (match memo to-print (print to-print) 0x)
    (ok true)
  )
)

(define-read-only (get-name) (ok "Stacky Balanced BTC"))
(define-read-only (get-symbol) (ok "sbBTC"))
(define-read-only (get-decimals) (ok u8))
(define-read-only (get-balance (account principal)) (ok (ft-get-balance sbBTC account)))
(define-read-only (get-total-supply) (ok (ft-get-supply sbBTC)))
