...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? saBTC amount recipient)
  )
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (try! (ft-transfer? saBTC amount sender recipient))
    (match memo to-print (print to-print) 0x)
    (ok true)
  )
)

(define-read-only (get-name) (ok "Stacky Aggressive BTC"))
(define-read-only (get-symbol) (ok "saBTC"))
(define-read-only (get-decimals) (ok u8))
(define-read-only (get-balance (account principal)) (ok (ft-get-balance saBTC account)))
(define-read-only (get-total-supply) (ok (ft-get-supply saBTC)))
